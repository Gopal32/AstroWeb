...
import { Clock3, PhoneOff, SendHorizontal } from "lucide-react";
import { useSession } from "@/context/SessionProvider";
import WaitingQueueModal from "@/app/components/WaitingQueuerModal";

const DEFAULT_CHAT_DURATION = 5 * 60;
...
const ChatPage = () => {
  ...
  const [timeLeft, setTimeLeft] = useState(
    (userServiceData?.timeSlot || 5) * 60 || DEFAULT_CHAT_DURATION
  );
  ...
  useEffect(() => {
    setTimeLeft((userServiceData?.timeSlot || 5) * 60 || DEFAULT_CHAT_DURATION);
      ...
      {isSocketConnected && (
        <div className="flex h-screen flex-col bg-[#FAF6ED] text-foreground transition-colors duration-300 dark:bg-background">
          <div className="absolute inset-0 -z-10 overflow-hidden">
            ...
            <div className="absolute bottom-0 right-0 h-80 w-80 rounded-full bg-sky-500/10 blur-3xl dark:bg-sky-400/10" />
          </div>
                    ...
                    <div className="flex h-full items-center justify-center">
                      <div className="max-w-md rounded-3xl border border-dashed border-border bg-background/70 px-6 py-8 text-center shadow-sm">
                        <p className="text-lg font-semibold text-foreground">
                          Your chat session is live
                        </p>
                        <p className="mt-2 text-sm leading-6 text-muted-foreground">
                          Ask your question to begin the conversation. New messages
                ...
                <div className="border-t border-border/70 bg-background/80 p-3 sm:p-4">
                  {isTimerExpired && (
                    <div className="mb-3 rounded-2xl border border-red-500/20 bg-red-500/10 px-4 py-3 text-sm text-red-600 dark:text-red-400">
                      Session time has ended. You can still review the chat, but
